...
class MetaSubject : SubjectBase, MetaObserver {
    ...
    func notifySubjectCreated(subject:Subject) {
        sendNotification(Notification(type:NotificationTypes.SUBJECT_CREATED, data:subject));
    }
    
    func notifySubjectDestroyed(subject:Subject) {
        sendNotification(Notification(type: NotificationTypes.SUBJECT_DESTROYED, data:subject));
    }
